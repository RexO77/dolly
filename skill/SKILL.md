...
7. **Check the camera.** `dolly studio <project> <clip> --text`. Done when every beat lands during a hold and the last hold is at least 0.8s.
8. **Hand it over.** Give the person `dolly studio <project> <clip>`: it serves the Studio until ctrl-c, so it is theirs to run. They direct, Save and Render there.
...
- Mark a **beat** (`h.beat`, or the `beat` option of `h.click`, `h.clickButton` and `hand.clickAt`) at every moment the camera cares about, and measure a **box** (`h.box`, `h.boxCss`) for every region it leans in on, where the region ends up.
...
4. Check each with `dolly studio <project> <clip> --text`, then hand the person the Studio, render and deliver as in steps 8 and 9 above.
...
| `the product reloaded during the take` | a hot update reset the app: files in the product changed while recording (another session editing it, a formatter, a build). Record once edits have stopped; `--takes 3` gives each clip three tries |
| `nothing visible on the page ...` or `no visible, enabled button ...` | look at `.dolly/<project>/<clip>-failed.png`, then `dolly inspect` again for the real label or selector |
...
| `this display's pixel ratio is 1, not 2` | the master will be 1x and leans will be soft; record on a retina display |
| `... is not installed, or not on your PATH` or `Chrome did not start` | run `dolly doctor` and the fix it prints |
| `there is no --x flag here` | run `dolly help <command>` for the flags it takes |
